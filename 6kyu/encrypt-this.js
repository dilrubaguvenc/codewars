...
var encryptThis = function(text) {
    return text.split` `.map(el => {
      ...
      if (el.length === 2) return el.charCodeAt(0) + el[1];
      if (el.length === 3) return el.charCodeAt(0) + el[2] + el[1];
      if (el.length > 3) return el.charCodeAt(0) + el[el.length -1] + el.slice(2, el.length -1) + el[1];
    }).join` `;
}
